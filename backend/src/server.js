...
function getMasteryState(user) {
  ...
  const completed = new Set(raw.completed || []);
  return completed;
}
...
app.patch("/me", authMiddleware, async (req,res) => {
  try {
    ...
    const t = name.trim();
    const ex = await prisma.user.findUnique({where:{name:t}});
    if (ex && ex.id!==req.userId) return res.status(400).json({message:"Этот ник уже занят"});
    const user = await prisma.user.update({where:{id:req.userId},data:{name:t}});
...
app.get("/mastery/status", authMiddleware, async (req,res) => {
  try {
    ...
    const pathsList = Object.values(MASTERY_PATHS).map(p => ({
      id:p.id, label:p.label, icon:p.icon, color:p.color,
      description:p.description, bonusDescription:p.bonusDescription, totalNodes:p.totalNodes,
    }));
    ...
    res.json({
      ...
      isComplete,
      statusChangesLeft:user.masteryStatusChangesLeft,
      paths:pathsList,
      path: path ? {
        id:path.id, label:path.label, icon:path.icon, color:path.color,
        statusLabel:path.statusLabel, bonusDescription:path.bonusDescription,
        finaleTitle:path.finale?.title, finaleDesc:path.finale?.description,
      } : null,
      // send node content so frontend can display tooltips
      nodeContent: Object.fromEntries(
        Object.entries(graph.nodes).map(([id,n]) => [id, {label:n.label, desc:n.desc, d:n.d, hidden:n.hidden||false}])
      ),
    });
  } catch(e){console.error(e);res.status(500).json({message:"Server error"});}
});
...
app.post("/mastery/choose", authMiddleware, async (req,res) => {
  try {
    ...
    if (user.masteryPath === pathId) {
      const cs = getMasteryState(user);
      if (cs.size === 0) return res.json({message:"Путь уже выбран"});
    }
    ...
    if (user.hasEverFinishedMastery && user.masteryPath) {
      ...
      await prisma.user.update({where:{id:req.userId}, data:{
        masteryPath:pathId, masteryNodeIndex:0,
        masteryChoices:JSON.stringify({completed:[]}),
        masteryStatusChangesLeft:{decrement:1}, hasEverFinishedMastery:false,
      }});
...
app.post("/mastery/complete-node", authMiddleware, async (req,res) => {
  try {
    ...
    await prisma.user.update({where:{id:req.userId}, data:{
      ...
      ...(justFinished ? {hasEverFinishedMastery:true} : {}),
    }});

    res.json({
      message: justFinished ? "Путь завершён!" : "Узел пройден",
      justFinished, leveledUp:level>user.level, newLevel:level,
      xpGained:rewards.xp, goldGained:rewards.gold,
...
app.patch("/tasks/:id/complete", authMiddleware, async (req,res) => {
  try {
    ...
    await prisma.user.update({where:{id:req.userId},data:{
      xp,level,gold:{increment:goldGain},
      ...(streakJustCompleted?{streak:newStreak,streakUpdatedDate:now}:{}),
      ...(freezeConsumed?{streakFreezeCount:{decrement:1}}:{}),
    }});
...
app.get("/shop", authMiddleware, async (req,res) => {
  try {
    ...
    res.json(items.map(item=>({...item,purchased:pIds.has(item.id),repeatable:REPEATABLE_SHOP_EFFECTS.includes(item.effect),locked:item.category!=="boost"&&!user.hasEverFinishedMastery})));
  } catch(e){console.error(e);res.status(500).json({message:"Server error"});}
});
...
app.post("/shop/:id/purchase", authMiddleware, async (req,res) => {
  try {
    ...
    const user=await prisma.user.findUnique({where:{id:req.userId}});
    if(item.category!=="boost"&&!user.hasEverFinishedMastery) return res.status(403).json({message:"Доступно после завершения пути Мастерства"});
    ...
    const ex=await prisma.purchase.findUnique({where:{userId_itemId:{userId:req.userId,itemId}}});
    if(ex) return res.status(400).json({message:"Already purchased"});
    const extra={};
    if(item.effect==="xp_boost_permanent") extra.xpBoostPermanent=true;
    if(item.effect==="gold_boost_permanent") extra.goldBoostPermanent=true;
    await prisma.$transaction([
      prisma.user.update({where:{id:req.userId},data:{gold:{decrement:item.price},...extra}}),
...
app.post("/friends", authMiddleware, async (req,res) => {
  try {
    ...
    await prisma.$transaction([prisma.friendship.create({data:{userId:req.userId,friendId:target.id}}),prisma.friendship.create({data:{userId:target.id,friendId:req.userId}})]);
...
app.delete("/friends/:id", authMiddleware, async (req,res) => {
  try {
    ...
    await prisma.$transaction([prisma.friendship.deleteMany({where:{userId:req.userId,friendId}}),prisma.friendship.deleteMany({where:{userId:friendId,friendId:req.userId}})]);
